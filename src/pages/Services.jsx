...
function AssessmentModal({ isOpen, onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (qualified) {
            // Client-side pixel Lead event with eventID for deduplication
            // @ts-ignore
        ...
        // 4. Trigger blueprint + queue dashboard (fire-and-forget)
        fetch('/api/generate-blueprint', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...payload, source: 'Services Page Assessment' }),
            keepalive: true,
...
function ContactSection() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (qualified) {
            ...
            } catch (err) { console.error('CAPI (existing pixel) error:', err); }
            ...
            } catch (err) { console.error('CAPI (qualified pixel) error:', err); }
        }
        ...
        fetch('/api/generate-blueprint', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...payload, source: 'Services Inline Form' }),
            keepalive: true,
        }).catch(err => console.error("Research trigger error", err));
        ...
        fetch('https://yrfobzuiqcuhylstiukn.supabase.co/functions/v1/queue-dashboard', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                clinic_name: form.clinicName || `${form.firstName} ${form.lastName}`.trim(),
                clinic_domain: form.website,
                services: form.services,
                city_state: form.cityState || '',
                contact_name: `${form.firstName} ${form.lastName}`.trim(),
                contact_email: form.email,
                monthly_budget: form.monthlyBudget,
                team_structure: form.teamStructure,
            }),
            keepalive: true,
        }).catch(err => console.error("Dashboard queue error", err));
                ...
                <FadeIn delay={0.15}>
                    <form onSubmit={handleSubmit} style={{
                        ...
                        <div>
                            ...
                            {/* Clinic Name */}
                            <div style={{ marginBottom: 20 }}>
                                <label style={sectionLabel}>Clinic Name *</label>
                                <input required style={inputStyle} placeholder="e.g. Clarity Ketamine" value={form.clinicName}
                                    onFocus={e => e.target.style.borderColor = BRAND.teal}
                                    onBlur={e => e.target.style.borderColor = '#e5e7eb'}
                                    onChange={e => handleChange('clinicName', e.target.value)} />
                            </div>

                            {/* Website + City/State row */}
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                                <div>
                                ...
                                <div>
                                    <label style={sectionLabel}>City & State *</label>
                                    <input required style={inputStyle} placeholder="e.g. Austin, TX" value={form.cityState}
                                        onFocus={e => e.target.style.borderColor = BRAND.teal}
                                        onBlur={e => e.target.style.borderColor = '#e5e7eb'}

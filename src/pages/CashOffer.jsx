...
function AssessmentModal({ isOpen, onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        // 4. Trigger blueprint (fire-and-forget)
        fetch('/api/generate-blueprint', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...payload, source: 'Cash Offer Assessment' }),
            keepalive: true,
        }).catch(err => console.error("Research trigger error", err));

        // 5. Queue dashboard (await this since it's fast ~1.5s, guarantees delivery)
        try {
            await fetch('https://yrfobzuiqcuhylstiukn.supabase.co/functions/v1/queue-dashboard', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
